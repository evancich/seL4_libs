...
typedef struct {
    ...
    /* Caps for the ethernet device. */
    uintptr_t ether_paddr;
    seL4_CPtr ether_irq_cap;
    seL4_CPtr ether_frame_cap;
    seL4_CPtr ether_notification_cap;
    seL4_CPtr ether_io_port_cap;
    ...
    /* paddr of untyped that each untyped cap corresponds to */
    uintptr_t paddrs[CONFIG_MAX_NUM_BOOTINFO_UNTYPED_CAPS];
    ...
    /* all procs will use only 0, except commproc which talks to everyone */
    seL4_CPtr shm_send_endpoint[MAX_SHM_COMMS];
    seL4_CPtr shm_receive_endpoint[MAX_SHM_COMMS];
    void *shm_region[MAX_SHM_COMMS];
} test_init_data_t;
